...
def affine_backward(dout, cache):
    ...
    """
    x, w, b = cache
    dx = np.dot(dout, w.T).reshape(x.shape)  # Derivative with respect to x
    dw = np.dot(x.reshape(x.shape[0], -1).T, dout)  # Derivative with respect to w
    db = np.sum(dout, axis=0)  # Derivative with respect to b
    return dx, dw, db
        ...
        for j in range(C):
            for k in range(H_out):
                for l in range(W_out):
                    # Compute the region to pool over
                    pool_region = x[i, j, k * stride:k * stride + pool_height, l * stride:l * stride + pool_width]
                    # Take the maximum value over the pool region
                    out[i, j, k, l] = np.max(pool_region)
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    window = x[n, c, start_i:end_i, start_j:end_j]
                    mask = (window == np.max(window))
                    dx[n, c, start_i:end_i, start_j:end_j] += dout[n, c, i, j] * mask
